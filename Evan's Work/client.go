...
func main()
{
	...
	go watchForConnIn(user, props, connect)
	for true
	{
...
func watchForConsoleIn(connect net.Conn)
{
	...
	for true
	{
		...
		util.CheckForError(err, "Lost Connection To Console")
...
{
	response := stdInMsgRegex.FindAllStringSubmatch(message, -1)
