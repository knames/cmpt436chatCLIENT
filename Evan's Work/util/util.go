...
const TIME_LAYOUT = "Jan 1 2015 -0600 GMT"
...
func (client *Client) Register() {
	// fmt.Println("Adding user to list")
...
func removeEntry(client *Client, clientArr []*Client) []*Client {
	...
	// fmt.Printf("User has been removed.")
	return retVal
}
...
func SendClientMessage(messageType string, message string, client *Client, thisClientOnly bool, props Properties) {
	...
	if thisClientOnly {
		...
		fmt.Fprintln(client.UserConnection, message)
		...
		for _, _client := range curClients {
			...
			if (thisClientOnly && _client.User == client.User) ||
				(!thisClientOnly && _client.User != "") {
